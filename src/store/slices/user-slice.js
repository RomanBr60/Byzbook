...
/*export const login = (username, password) => createAsyncThunk('user/login', async () => { // Corrected action type
  const res = await getPost('/login', {username: username, password: password}); // Corrected function name
  return res;
});*/
  ...
  extraReducers: (builder) => {
    ...
    /*.addCase(login.pending, (state) => {
      ...
      .addCase(login.fulfilled, (state, action) => {
        state.status = 'ready';
        state.username = action.payload.username;
        state.role = !action.payload.username || action.payload.username === '' ? '' : 'admin';
      });*/
  },
});
